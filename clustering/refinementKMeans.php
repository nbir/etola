...
class kMeans
{
	...
	{
		$this->formInitialClusters($corpus);
	...
	private function formInitialClusters($clusters)
	{
		//$id=0;
		$k=0;
		/* Load all documents in input corpus into the document_set 
		 * array without changing document ids. Also assignes cluster 
		 * ids to all documents in clusters with more than one 
		 * document. */
		foreach($clusters as $cluster)
		{
			if(count($cluster)>1)
			{
				...
				foreach($cluster as $doc_id=>$document)
				{
					//$id++;
			...
			else
			{
				foreach($cluster as $document)
				{
					...
					/* For clusters with one documents, assign no 
					 * cluster id. In these cases, clusterid is 0. */
