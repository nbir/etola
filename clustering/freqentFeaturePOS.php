...
class FreqentFeaturePOS
{
	...
	private function createListOfTerms()
	{
		foreach($this->document_set as $document)
		{
			...
			{
				if($this->checkPOS($term["tag"]))
				{
					$feature=$this->applyPreprocessing($term["token"]);
	...
	private function applyPreprocessingToCorpus()
	{
		foreach($this->document_set as $doc_id=>$document)
		{
			...
			{
				$this->document_set[$doc_id][$term_id]["token"]=$this->applyPreprocessing($this->document_set[$doc_id][$term_id]["token"]);
			}
		}
	}
